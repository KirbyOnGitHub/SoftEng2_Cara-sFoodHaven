...
function fillForm(inputID, tableID, dataName, formID) {
    ...
    // Iterate through each row to find a matching input value
    let found = false; // Track if a match was found
    rows.forEach(row => {
        const rowData = JSON.parse(row.getAttribute(`data-${dataName}`));
        
        // Check if the ID matches
        if (rowData.id === inputValue) {
            // Check if the row is already clicked
            if (row.classList.contains("clickedTableRow")) {
                showNotification(`The form is already filled with ID: ${inputValue}.`, formID);
                found = true; // Mark as found
                return; // Exit if the row is already clicked
            }
    ...
    // If no match was found, show a notification
    if (!found) {
...
function addIngredientToList() {
    ...
    rows.forEach(row => {
        ...
        if (ingredientData[1].trim() === selectedIngredient) {
            selectedRow = row;
            unitText = ingredientData[3].trim();  // Assuming the unit is in the data attribute
        }
    });
    ...
    ingredientNameElement.textContent = selectedIngredient;
